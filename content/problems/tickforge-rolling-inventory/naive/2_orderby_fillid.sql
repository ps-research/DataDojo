-- NAIVE 2: running total ordered by fill_id, assuming the surrogate id tracks
-- chronological order. Fills are stamped out of id order (a later id can carry an
-- earlier fill_time), so the running series is attributed to the wrong points.
WITH ff AS (
...
SELECT instrument_id, session_date, fill_time, fill_id, signed_qty,
  SUM(signed_qty) OVER (PARTITION BY instrument_id ORDER BY fill_id
                        ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS running_inventory,
  ROUND(SUM(signed_cash) OVER (PARTITION BY instrument_id ORDER BY fill_id
