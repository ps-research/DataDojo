-- NAIVE 3: ROW_NUMBER instead of RANK for the volume ranking. The scripted twin
-- names print identical volume and must share a rank; ROW_NUMBER breaks the tie
-- arbitrarily and renumbers every name below it.
WITH taker AS (
...
SELECT ROW_NUMBER() OVER (ORDER BY taker_volume DESC) AS vol_rank,
