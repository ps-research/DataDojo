-- NAIVE 1: no de-duplication of business-duplicate (double-booked) fills.
-- Each replayed print inflates taker_volume and skews VWAP. (Duplicate fills are
-- injected only at full-landmine scale, so this matches on the mild visible sample
-- but diverges on the hidden black fixture.)
WITH taker AS (
...
ff AS (
    ...
    FROM taker WHERE 1 = 1  -- NO DEDUP: double-booked fills counted twice
),
vq AS (
