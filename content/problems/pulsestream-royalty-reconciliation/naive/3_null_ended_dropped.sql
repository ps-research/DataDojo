...
-- `play_date <= s.ended_at` is UNKNOWN when ended_at is NULL, so every play made
-- under a still-active (open-ended) subscription is dropped. Those are exactly the
-- current premium/family listeners, so a large share of royalty vanishes and the
-- whole reconciliation is wrong. NULL ended_at means "still active", not "excluded".
WITH play_dates AS (
...
active_plan AS (
    ...
    JOIN subscriptions s
     ...
     AND pd.play_date <= s.ended_at            -- BUG: drops open-ended (NULL) subs
),
paid_plays AS (
