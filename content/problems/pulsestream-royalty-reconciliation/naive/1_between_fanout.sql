...
play_royalty AS (
    ...
    LEFT JOIN royalty_rates rc
          ...
          AND CONCAT(rc.effective_from, '') <= pd.play_date
          AND (rc.effective_to IS NULL OR pd.play_date < CONCAT(rc.effective_to, ''))
    LEFT JOIN royalty_rates rg
           ON rg.plan = s.plan AND rg.country IS NULL
          AND CONCAT(rg.effective_from, '') <= pd.play_date
          AND (rg.effective_to IS NULL OR pd.play_date < CONCAT(rg.effective_to, ''))
),
computed AS (
