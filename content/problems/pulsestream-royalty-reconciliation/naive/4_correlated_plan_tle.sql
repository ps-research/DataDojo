...
-- subquery (ORDER BY precedence ... LIMIT 1) evaluated once PER PLAY. This produces
-- the SAME answer as the reference but is O(plays x subscriptions): on the 3M-row
-- hidden fixture it re-scans a listener's subscriptions for every one of their
-- plays and blows the time limit. The reference resolves all plans in a single
-- windowed pass. (Output matches the reference on small inputs; see problem.json
-- notes -- this is the designated naive-slow for the section-6 TLE calibration.)
WITH play_dates AS (
