...
--
-- Each stream earns the per-play rate of the listener's ACTIVE plan (highest plan
-- precedence when periods overlap), in the listener's market (country-specific rate,
-- else the global fallback), under the rate epoch whose half-open
-- [effective_from, effective_to) window covers the play date. free/trial earn nothing.
-- Attribute to the track's artist, total by (artist, accounting month), reconcile
-- against artist_payouts, and surface every artist-month not paid within one cent.
--
-- Determinism: royalties are summed in exact INTEGER micro-dollars (rate * 1e6),
-- then rounded to cents, so results are identical across engines (no float drift).
-- Dates are coerced to ISO text via CONCAT(col,'') and compared lexically. Verified
-- identical on sqlite, duckdb, postgres.
WITH play_dates AS (
