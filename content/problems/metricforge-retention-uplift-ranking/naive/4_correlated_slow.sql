...
qual AS (SELECT cw.experiment_id, cw.experiment_key, cw.variant, cw.user_id, cw.assigned_ts, cw.primary_metric
  FROM clean_wk cw
  WHERE (SELECT COUNT(DISTINCT uwk.sess_wk) FROM uwk WHERE uwk.user_id=cw.user_id
         AND uwk.sess_wk BETWEEN cw.assign_wk+1 AND cw.assign_wk+2) = 2),
conv AS (SELECT q.experiment_id, q.experiment_key, q.variant,
    CASE WHEN EXISTS (SELECT 1 FROM events e WHERE e.user_id=q.user_id AND e.event_type=q.primary_metric AND e.event_ts>q.assigned_ts) THEN 1 ELSE 0 END AS converted
  FROM qual q),
...
pivot AS (SELECT c.experiment_id, c.experiment_key,
    COALESCE(ct.n_users,0) control_qualified, COALESCE(ct.n_conv,0) control_conv,
...
SELECT ROW_NUMBER() OVER (ORDER BY (treatment_rate-control_rate) DESC, qualified_size DESC, experiment_key ASC) AS rank_pos,
