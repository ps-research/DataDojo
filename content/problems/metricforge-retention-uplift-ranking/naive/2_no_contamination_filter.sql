...
island AS (SELECT cw.experiment_id, cw.experiment_key, cw.primary_metric, cw.variant, cw.user_id, cw.assigned_ts FROM clean_wk cw JOIN uwk uw ON uw.user_id=cw.user_id AND uw.sess_wk BETWEEN cw.assign_wk+1 AND cw.assign_wk+2 GROUP BY cw.experiment_id, cw.experiment_key, cw.primary_metric, cw.variant, cw.user_id, cw.assigned_ts HAVING COUNT(DISTINCT uw.sess_wk)=2),
conv AS (SELECT i.experiment_id, i.experiment_key, i.variant, i.user_id, MAX(CASE WHEN e.event_type=i.primary_metric AND e.event_ts>i.assigned_ts THEN 1 ELSE 0 END) AS converted FROM island i LEFT JOIN events e ON e.user_id=i.user_id GROUP BY i.experiment_id, i.experiment_key, i.variant, i.user_id),
