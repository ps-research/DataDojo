...
comp AS (SELECT experiment_id, experiment_key, start_date, COALESCE(end_date,'2025-03-31') AS end_eff, primary_metric FROM experiments WHERE status='completed' AND end_date IS NOT NULL),
...
multi AS (SELECT DISTINCT a.experiment_id, a.user_id FROM experiment_assignments a JOIN comp c ON c.experiment_id=a.experiment_id JOIN experiment_assignments a2 ON a2.user_id=a.user_id AND a2.experiment_id<>a.experiment_id JOIN allexp y ON y.experiment_id=a2.experiment_id WHERE y.start_date<=c.end_eff AND c.start_date<=y.end_eff),
...
clean AS (SELECT a.experiment_id, c.experiment_key, c.primary_metric, a.user_id, MIN(a.variant) variant, MIN(a.assigned_ts) assigned_ts
  FROM experiment_assignments a JOIN comp c ON c.experiment_id=a.experiment_id JOIN users u ON u.user_id=a.user_id AND u.is_internal=0
...
clean_wk AS (SELECT cl.experiment_id, cl.experiment_key, cl.primary_metric, cl.user_id, cl.variant, cl.assigned_ts, ca.wk AS assign_wk FROM clean cl JOIN cal ca ON cl.assigned_ts>=ca.wk_start AND cl.assigned_ts<ca.wk_end),
...
-- NAIVE: total post-assignment active-week COUNT >= K (admits gaps, not a consecutive island)
qual AS (SELECT cw.experiment_id, cw.experiment_key, cw.variant, cw.user_id, cw.assigned_ts, cw.primary_metric FROM clean_wk cw
  WHERE (SELECT COUNT(DISTINCT uwk.sess_wk) FROM uwk WHERE uwk.user_id=cw.user_id AND uwk.sess_wk > cw.assign_wk) >= 2),
conv AS (SELECT q.experiment_id, q.experiment_key, q.variant, CASE WHEN EXISTS (SELECT 1 FROM events e WHERE e.user_id=q.user_id AND e.event_type=q.primary_metric AND e.event_ts>q.assigned_ts) THEN 1 ELSE 0 END AS converted FROM qual q),
...
pivot AS (SELECT c.experiment_id, c.experiment_key, COALESCE(ct.n_users,0) control_qualified, COALESCE(ct.n_conv,0) control_conv, COALESCE(tr.n_users,0) treatment_qualified, COALESCE(tr.n_conv,0) treatment_conv FROM comp c LEFT JOIN agg ct ON ct.experiment_id=c.experiment_id AND ct.variant='control' LEFT JOIN agg tr ON tr.experiment_id=c.experiment_id AND tr.variant='treatment'),
rated AS (SELECT experiment_key, control_qualified, treatment_qualified, control_qualified+treatment_qualified AS qualified_size, 1.0*control_conv/NULLIF(control_qualified,0) AS control_rate, 1.0*treatment_conv/NULLIF(treatment_qualified,0) AS treatment_rate FROM pivot WHERE control_qualified>0 AND treatment_qualified>0)
SELECT ROW_NUMBER() OVER (ORDER BY (treatment_rate-control_rate) DESC, qualified_size DESC, experiment_key ASC) AS rank_pos, experiment_key, control_qualified, treatment_qualified, qualified_size, ROUND(control_rate,6) control_rate, ROUND(treatment_rate,6) treatment_rate, ROUND(treatment_rate-control_rate,6) uplift FROM rated ORDER BY rank_pos;
