-- NAIVE 1: plain SUM of signed quantity with NO split re-basing. After any split
-- ex-date the reconstructed share count is off by the split factor and fails to
-- reconcile with the honest snapshot. (Splits are present at every scale, so this
-- is wrong on the visible sample too.)
WITH splits AS (
