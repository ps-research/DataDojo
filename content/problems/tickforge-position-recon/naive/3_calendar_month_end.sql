-- NAIVE 3: month-ends chosen by calendar day-of-month (>= 28) rather than the
-- trading calendar. It misses real month-ends whose last session is not the 28th-
-- 31st (e.g. a final session landing on the 1st) and would misplace the boundary
-- around the 2024-02-29 leap session; snapshot dates then find no reconstruction.
WITH splits AS (
...
dd AS (
    ...
    FROM (
      SELECT account_id, instrument_id, session_date, side, fill_quantity,
...
month_ends AS (
    -- WRONG: calendar guess -- any session whose day-of-month is 28 or later
    SELECT session_date AS as_of_date
    FROM trading_days
    WHERE CAST(substr(session_date,9,2) AS INTEGER) >= 28
),
...
snap AS (
    ...
    SELECT account_id, instrument_id, as_of_date, snap_qty FROM (
      SELECT account_id, instrument_id, as_of_date, quantity AS snap_qty,
             ROW_NUMBER() OVER (PARTITION BY account_id, instrument_id, as_of_date
                                ORDER BY position_id) AS rn
      FROM positions
    ) p WHERE rn=1
)
