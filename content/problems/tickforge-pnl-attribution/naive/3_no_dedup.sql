-- NAIVE 3: no de-duplication of double-booked fills (injected only at full scale).
-- Matches on the mild visible sample; on the hidden red fixture the phantom fills
-- corrupt both the position and the trading PnL.
...
dd AS (  -- de-duplicated fills, signed, native basis
    SELECT t.account_id, t.instrument_id, t.session_date,
           CASE WHEN t.side='BUY' THEN t.fill_quantity ELSE -t.fill_quantity END AS sq,
           (CASE WHEN t.side='BUY' THEN t.fill_quantity ELSE -t.fill_quantity END) * t.fill_price AS sqn
    FROM fills t  -- NO DEDUP of double-booked fills
),
daytrade AS (
