-- NAIVE 2: applies splits and dividends on the announcement date (announced_at)
-- rather than the ex-date. Announcements do not line up with ex-dates (and some
-- are retroactive), so the adjustment lands on the wrong session.
...
splits AS (
    SELECT instrument_id, substr(announced_at,1,10) AS ex_date,
...
),  -- (announced_at substituted for ex_date below)
divs AS (
    SELECT instrument_id, substr(announced_at,1,10) AS ex_date, SUM(cash_amount) AS div_cash
    FROM corporate_actions WHERE action_type='DIVIDEND' AND cash_amount IS NOT NULL
    GROUP BY instrument_id, substr(announced_at,1,10)
),
