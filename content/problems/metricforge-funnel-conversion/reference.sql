...
fl AS (
  SELECT channel,
    ...
    CASE WHEN t1 IS NOT NULL AND t2 IS NOT NULL AND t2>=t1
          AND t3 IS NOT NULL AND t3>=t2 THEN 1 ELSE 0 END AS r3,
    CASE WHEN t1 IS NOT NULL AND t2 IS NOT NULL AND t2>=t1
          AND t3 IS NOT NULL AND t3>=t2
          AND t4 IS NOT NULL AND t4>=t3 THEN 1 ELSE 0 END AS r4
  FROM sess
)
SELECT channel AS referral_channel,
       SUM(r1) AS entered,
       SUM(r2) AS reached_checkout,
       SUM(r3) AS reached_payment,
       SUM(r4) AS reached_purchase,
       ROUND(1.0*SUM(r2)/NULLIF(SUM(r1),0), 6) AS cr_checkout,
       ROUND(1.0*SUM(r3)/NULLIF(SUM(r2),0), 6) AS cr_payment,
       ROUND(1.0*SUM(r4)/NULLIF(SUM(r3),0), 6) AS cr_purchase
FROM fl
GROUP BY channel
