...
active AS (
  SELECT DISTINCT c.wk AS wk, c.wk_start AS wk_start, e.user_id AS user_id
  FROM events e JOIN users u ON u.user_id=e.user_id
  JOIN cal c ON e.event_ts>=c.wk_start AND e.event_ts<c.wk_end
  WHERE u.is_internal=0
),
wau AS (SELECT wk, COUNT(*) AS wau FROM active GROUP BY wk),
