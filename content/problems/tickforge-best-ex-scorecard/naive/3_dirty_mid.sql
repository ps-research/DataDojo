-- NAIVE 3: arrival and markout mids taken with no validity filter, so NULL-side,
-- crossed/locked, and one-sided (size-0) quotes corrupt both mids.
WITH taker AS (
...
vq AS (
    ...
    FROM quotes
    WHERE 1=1  -- NO VALIDITY FILTER: NULL-side, crossed, one-sided quotes pollute mids
),
ev AS (
