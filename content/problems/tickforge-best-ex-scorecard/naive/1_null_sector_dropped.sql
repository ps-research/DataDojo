-- NAIVE 1: treats NULL-sector (unclassified) names as noise and filters them out,
-- so the unclassified cohort silently disappears from the scorecard. (No NULL
-- sectors exist at the small sample scale, so this matches on the visible sample
-- but drops real names on the hidden red fixture -- the classic NULL cohort trap.)
WITH taker AS (
...
inst_agg AS (
    SELECT i.instrument_id, i.symbol, i.sector AS sector_cohort,
    ...
    WHERE i.sector IS NOT NULL  -- WRONG: drops the unclassified cohort entirely
