...
usr AS (
  SELECT u.user_id, c.wk AS signup_wk, c.wk_start AS cohort_start,
         CAST(strftime('%W', u.signup_ts) AS INTEGER) AS signup_woy
  FROM users u JOIN cal c ON u.signup_ts>=c.wk_start AND u.signup_ts<c.wk_end
  WHERE u.is_internal=0
),
uwk AS (
  SELECT DISTINCT e.user_id AS user_id, CAST(strftime('%W', e.event_ts) AS INTEGER) AS active_woy
  FROM events e WHERE e.user_id IN (SELECT user_id FROM usr)
),
act AS (
  SELECT usr.signup_wk, usr.cohort_start, uwk.user_id, (uwk.active_woy - usr.signup_woy) AS off
  FROM usr JOIN uwk ON uwk.user_id=usr.user_id
  WHERE (uwk.active_woy - usr.signup_woy) BETWEEN 0 AND 8
),
