...
SELECT cs.cohort_start AS cohort_week, o.off AS week_offset, cs.cohort_size,
  (SELECT COUNT(DISTINCT u2.user_id) FROM usr u2
     WHERE u2.signup_wk=cs.signup_wk
       AND EXISTS (SELECT 1 FROM events e JOIN cal c2 ON e.event_ts>=c2.wk_start AND e.event_ts<c2.wk_end
                   WHERE e.user_id=u2.user_id AND c2.wk = cs.signup_wk + o.off)) AS active_users,
  ROUND(1.0*(SELECT COUNT(DISTINCT u2.user_id) FROM usr u2
     WHERE u2.signup_wk=cs.signup_wk
       AND EXISTS (SELECT 1 FROM events e JOIN cal c2 ON e.event_ts>=c2.wk_start AND e.event_ts<c2.wk_end
                   WHERE e.user_id=u2.user_id AND c2.wk = cs.signup_wk + o.off))/NULLIF(cs.cohort_size,0),6) AS retention_rate
