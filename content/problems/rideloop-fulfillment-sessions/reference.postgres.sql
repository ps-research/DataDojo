-- R1 - PostgreSQL variant. Same sessionization as reference.sql; the temporal
-- deltas use EXTRACT(EPOCH ...) seconds, and the median is the exact
-- PERCENTILE_CONT(0.5) (linear interpolation = mean of the two central order
-- statistics for even counts, matching the reference).
WITH ordered AS (
...
flagged AS (
    SELECT ordered.*,
        CASE WHEN prev_ts IS NULL
                  OR EXTRACT(EPOCH FROM (CAST(request_ts AS TIMESTAMP) - CAST(prev_ts AS TIMESTAMP))) > 300
...
sessions AS (
    SELECT a.rider_id, a.session_no, g.city, a.fulfilled,
        EXTRACT(EPOCH FROM (CAST(fc.dropoff_ts AS TIMESTAMP) - CAST(a.start_ts AS TIMESTAMP))) / 60.0
            AS latency_min
    FROM session_agg a
...
median AS (
    SELECT city, PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY latency_min) AS median_min
...
SELECT s.city,
    ...
    ROUND(CAST(m.median_min AS NUMERIC), 2) AS median_minutes_to_completion
FROM sessions s
